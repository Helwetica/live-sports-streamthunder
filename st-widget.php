...
Plugin URI: http://www.streamthunder.com
...
function sthunder_wp_shortcode($atts)
{
    ...
    if($atts)
    {
        ...
        foreach($atts as $name=>$value)
        {
            ...
            if(!in_array($name, array('widget_type', 'widget_scrollbars'))) // ignore special settings
            {
                if(!intval($atts['cs']) || in_array($name, array('cs', 'l', 'l2', 'sp')))// disable extra styles for custom settings
                {
    ...
    $iframe_url = isset($options['widget_type']) && $options['widget_type'] ? '' : 'widget_fp.php';
    $sthunder_link = isset($options['sthunder_link']) ? $options['sthunder_link'] : 1;



    $widget = '<div class="st_widget live_streams">
    <iframe src="http://widget.streamthunder.com/'.$iframe_url.'?d=1&s=1'.$added_attributes.'" width="100%" height="'.htmlspecialchars($widget_height).'" scrolling="'.($iframe_scrolling?'auto':'no').'" align="top" frameborder="0">Your browser does not support frames, so you will not be able to view this page.</iframe>
    '.($sthunder_link?'<br><a style="float: right;font-size: 10px;color: #6b6b6b;"  href="http://www.streamthunder.com" target="_blank">Live Stream Sports</a></div>':'');
    return $widget;
}
...
function sthunder_backend_footer()
{
    echo '
    <hr>
    <a href="http://www.streamthunder.com" target="_blank">Live Stream Sports Widget</a>
    </div>';
}
